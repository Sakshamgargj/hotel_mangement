...
const Navbar = () => {
    ...
    return (
        ...
        >
                ...
                <nav className="hidden  md:flex items-center space-x-6">
                    {navItems.map((item) => (
                        <Link
                            key={item.to}
                            href={item.to}
                            className={`cursor-pointer text-[13px] font-semibold tracking-wider whitespace-pre-line hover:shadow-[0_0_15px_rgba(255,255,255,0.6)] ${navStyle} ${router.pathname === item.to ? navStyleSelected : ""}`}
                        >
                            {item.label}
                        </Link>
                    ))}
            ...
            <div
                className={`fixed inset-0 mt-20 bg-black bg-opacity-95 flex flex-col items-center justify-center space-y-4 text-white text-lg z-40 transform transition-all duration-500 ${
